...
function getCommandHelp(group, command) {
    if (group === "datasets") {
        if (command === "ingest") {
...
function buildModelMergeOptions(options) {
    return {
...
function buildModelRenameOptions(options) {
    return {
        sbv2Root: options.sbv2Root,
...
function datasetPathRoles(dataset, job) {
    return {
...
function trainingPathRoles(plan, job) {
    return {
        ...
        if (parsed.group === "models") {
            ...
            if (parsed.command === "merge-plan") {
                ...
                if (options.json) {
                    ...
                    for (const error of result.compatibility.errors)
                        writeLine(stdout, `error: ${error}`);
                    for (const warning of result.compatibility.warnings)
                        writeLine(stdout, `warning: ${warning}`);
            ...
            if (parsed.command === "style-merge-plan") {
                ...
                if (options.json) {
                    ...
                    writeLine(stdout, `compatible: ${result.compatibility.compatible ? "yes" : "no"}`);
                    for (const row of result.styleRows)
                        writeLine(stdout, `style: ${row.styleA} + ${row.styleB} -> ${row.outputStyle}`);
                    for (const warning of result.compatibility.warnings)
                        writeLine(stdout, `warning: ${warning}`);
                    for (const error of result.compatibility.errors)
                        writeLine(stdout, `error: ${error}`);
            ...
            if (parsed.command === "rename-plan") {
                ...
                if (options.json) {
                    ...
                    for (const warning of plan.compatibility.warnings)
                        writeLine(stdout, `warning: ${warning}`);
                    for (const error of plan.compatibility.errors)
                        writeLine(stdout, `error: ${error}`);
                }
            ...
            if (parsed.command === "rename-run") {
                ...
                if (options.json) {
                    ...
                    if (result.summary.refresh) {
                        writeLine(stdout, `refresh: ${result.summary.refresh.foundNewInModelsInfo && !result.summary.refresh.foundOldInModelsInfo ? "verified" : "failed"}`);
                    }
                    writeLine(stdout, `summary: ${result.job.outputDir}/summary.json`);
                    writeLine(stdout, `job: ${result.job.jobId}`);
                    writeLine(stdout, `job log: ${result.job.logPath}`);
