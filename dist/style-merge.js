...
export async function createStyleMergePlan(options) {
    const recipePath = resolveUserPath(options.recipePath);
    const recipe = parseStyleMergeRecipe(JSON.parse(await readFile(recipePath, "utf8")));
...
export async function runStyleMerge(options) {
...
function parseStyleMergeRecipe(value) {
...
async function inspectStyleMergeInput(assetsRoot, modelName, label) {
    ...
    if (!(await isDirectory(modelDir)))
        throw new Error(`${label} assets directory was not found: ${modelDir}`);
...
function bufferDataIsTruncated(buffer, header) {
...
function parseNpy(buffer) {
    ...
    const shape = shapeText
        ...
        .filter(Boolean)
        .map(Number);
    if (!shape.length || !shape.every((part) => Number.isInteger(part) && part >= 0))
        return undefined;
...
function buildStyleMergeInputSummary(plan, extra) {
    return {
        outputModelName: plan.outputModelName,
        outputDir: plan.outputDir,
        modelA: plan.modelA.modelName,
...
function styleMergeRecipePath(outputDir) {
...
function readNumStyles(config, filePath) {
    const data = config.data;
...
function normalizeStyleWeight(value) {
...
function modelInfoContains(models, modelName) {
