...
低レベル knob の `sdp_ratio`, `noise`, `noisew`, `auto_split`, `split_interval` は OpenClaw agent が扱う per-request override からは受け付けない。必要な場合は operator が provider config の既定値として固定する。agent surface に必要になった場合は別 issue で whitelist、range、policy mapping を決めてから追加する。
