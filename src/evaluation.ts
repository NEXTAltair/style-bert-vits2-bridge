...
import { Sbv2Client, type Sbv2SynthesizeParams } from "./sbv2-client.js";
...
export const DEFAULT_EVALUATION_TEST_CASES: Sbv2EvaluationTestCase[] = [
...
export async function evaluateModelCandidate(options: EvaluateModelOptions): Promise<EvaluationResult> {
  ...
  const evaluation: Sbv2EvaluationManifest = {
    ...summary,
    createdAt,
    baseUrl: sanitizeBaseUrl(options.baseUrl),
    candidate,
...
export async function readEvaluationManifest(evaluationPath: string): Promise<Sbv2EvaluationManifest> {
  const parsed = JSON.parse(await readFile(resolveUserPath(evaluationPath), "utf8")) as unknown;
...
export async function updateEvaluationNote(options: EvaluationNoteOptions): Promise<Sbv2EvaluationManifest> {
  ...
  const updated: Sbv2EvaluationManifest = {
    ...evaluation,
    ...summary,
    notes,
  };
...
export function buildEvaluationSummary(
  ...
  let recommendation: Sbv2EvaluationRecommendation;
  if (failureCount === samples.length || explicitReject) {
    ...
    recommendation = "hold";
    if (failureCount > 0) rationale.push(`${failureCount} sample(s) failed generation or WAV checks.`);
    if (warningCount > 0) rationale.push(`${warningCount} mechanical warning(s) need review.`);
    if (explicitHold) rationale.push("A human listening note marked at least one sample as hold.");
  } else {
...
export function analyzeWavBuffer(buffer: Buffer): Sbv2EvaluationAudioCheck {
  ...
  if (!fmt || fmt.size < 16) errors.push("WAV fmt chunk is missing or too small");
  if (!data || data.size <= 0) errors.push("WAV data chunk is missing or empty");
  ...
  if (readableFmt) {
    if (audioFormat !== 1) errors.push("WAV audio format must be PCM");
    if (!channels) errors.push("WAV channel count must be greater than zero");
    if (!sampleRate) errors.push("WAV sample rate must be greater than zero");
    if (!bitsPerSample) {
  ...
  if (durationSec !== undefined && durationSec < 0.15) warnings.push("WAV duration is very short");

  let nearZeroRatio: number | undefined;
  if (data && data.size > 0) {
    ...
    if (nearZeroRatio > 0.98) warnings.push("WAV appears mostly silent");
  }
...
async function readEvaluationTestSet(filePath: string): Promise<Sbv2EvaluationTestCase[]> {
...
async function readCandidateVoiceDefaults(candidate: Sbv2ModelCandidate): Promise<{
  speakerName?: string;
  style?: string;
}> {
  try {
    const config = JSON.parse(await readFile(candidate.configJsonPath, "utf8")) as unknown;
    if (!isRecord(config) || !isRecord(config.data)) return {};
    return {
...
function buildSynthesizeParams(
  modelName: string,
  defaults: { speakerName?: string; style?: string },
  testCase: Sbv2EvaluationTestCase,
): Sbv2SynthesizeParams {
  return {
    text: testCase.text,
    modelName,
...
function findWavChunk(buffer: Buffer, id: string): { offset: number; size: number } | undefined {
  let offset = 12;
  while (offset + 8 <= buffer.length) {
    const chunkId = buffer.toString("ascii", offset, offset + 4);
    const size = buffer.readUInt32LE(offset + 4);
    const dataOffset = offset + 8;
    if (dataOffset + size > buffer.length) return undefined;
...
function calculateNearZeroRatio(payload: Buffer, bitsPerSample: number | undefined): number {
  if (bitsPerSample === 16 && payload.length >= 2) {
    let nearZero = 0;
    let total = 0;
    for (let offset = 0; offset + 1 < payload.length; offset += 2) {
      total += 1;
      if (Math.abs(payload.readInt16LE(offset)) <= 128) nearZero += 1;
    }
    return total ? nearZero / total : 1;
  }
  let nearZero = 0;
  for (const value of payload) {
    if (value >= 126 && value <= 130) nearZero += 1;
  }
...
function firstPreferredStyle(value: unknown): string | undefined {
  if (!isRecord(value)) return undefined;
...
function isEvaluationManifest(value: unknown): value is Sbv2EvaluationManifest {
  return (
    isRecord(value) &&
