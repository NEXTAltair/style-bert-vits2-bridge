...
async function validateOutputConfig(configJsonPath: string, errors: string[]): Promise<void> {
  try {
    const config = await readConfig(configJsonPath);
...
async function hasNonEmptySafetensors(directoryPath: string): Promise<boolean> {
  try {
