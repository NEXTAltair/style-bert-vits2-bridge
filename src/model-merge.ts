...
import { listModelCandidates, type Sbv2ModelCandidate } from "./model-registry.js";
...
export interface Sbv2ModelMergeInput {
  ...
  safetensorsTensors: Record<string, { dtype?: string; shape: number[] }>;
}
...
export type ModelMergeCommandRunner = (
  executable: string,
  args: string[],
  options: ModelMergeCommandOptions,
) => Promise<ModelMergeCommandResult>;
...
export async function createModelMergePlan(options: ModelMergePlanOptions): Promise<Sbv2ModelMergePlan> {
  ...
  const styleRecipe = styleRecipePath
    ? parseModelMergeStyleRecipe(JSON.parse(await readFile(styleRecipePath, "utf8")) as unknown)
    : undefined;
...
export async function runModelMerge(options: ModelMergeRunOptions): Promise<ModelMergeRunResult> {
...
async function resolveModelSafetensors(modelDir: string, modelFile: string | undefined, label: string): Promise<string> {
  if (modelFile) {
  ...
  if (!files.length) throw new Error(`${label} has no non-empty .safetensors file in ${modelDir}`);
...
async function checkCompatibility(args: {
  ...
  outputModelName: string;
  outputDir: string;
  inputModels: Sbv2ModelMergePlan["inputModels"];
  styleErrors?: string[];
  styleWarnings?: string[];
}): Promise<Sbv2ModelMergeCompatibilityReport> {
  const errors: string[] = [];
...
function parseModelMergeStyleRecipe(value: unknown): Sbv2ModelMergeStyleRecipe {
...
function buildStyleMergePlan(args: {
  method: Sbv2ModelMergeMethod;
  inputModels: Sbv2ModelMergePlan["inputModels"];
  styleRecipe: Sbv2ModelMergeStyleRecipe;
}): { styleRows: Sbv2ModelMergeStyleRow[]; outputStyle2id: Record<string, number>; errors: string[]; warnings: string[] } {
  const errors: string[] = [];
  const warnings: string[] = [];
  const outputStyle2id = createStringNumberRecord();
  const styleRows: Sbv2ModelMergeStyleRow[] = [];
  ...
  for (const [index, style] of args.styleRecipe.styles.entries()) {
    ...
    if (styleAIndex === undefined) errors.push(`style "${style.styleA}" was not found in model A (${args.inputModels.a.modelName})`);
    if (styleBIndex === undefined) errors.push(`style "${style.styleB}" was not found in model B (${args.inputModels.b.modelName})`);
    ...
    for (const [label, value] of [
      ["styleA", style.styleA],
      ["styleB", style.styleB],
      ...(style.styleC ? ([["styleC", style.styleC]] as const) : []),
...
async function applyMergedStyles(plan: Sbv2ModelMergePlan): Promise<void> {
  if (!plan.styleRows || !plan.outputStyle2id) return;
...
async function buildMergedStyleVectors(plan: Sbv2ModelMergePlan): Promise<Float32Array> {
  if (!plan.styleRows) return new Float32Array();
...
function mergeStyleValue(plan: Sbv2ModelMergePlan, valueA: number, valueB: number, valueC: number): number {
  ...
  if (plan.method === "add-diff") return valueA + weight * (valueB - valueC);
  if (plan.method === "add-null") return valueA + weight * valueB;
...
function buildOutputStyleRecipe(plan: Sbv2ModelMergePlan): Record<string, unknown> {
  return {
    schemaVersion: 1,
...
function compareSafetensorsTensorMaps(inputs: Sbv2ModelMergeInput[]): string[] {
...
async function runSbv2Command(executable: string, args: string[], options: ModelMergeCommandOptions): Promise<ModelMergeCommandResult> {
  return new Promise((resolve, reject) => {
    const child = spawn(executable, args, { cwd: options.cwd, stdio: ["ignore", "pipe", "pipe"] });
    child.stdout?.setEncoding("utf8");
    child.stderr?.setEncoding("utf8");
    child.stdout?.on("data", (chunk: string) => options.onOutput?.("stdout", chunk));
    child.stderr?.on("data", (chunk: string) => options.onOutput?.("stderr", chunk));
...
async function runAndLogMergeCommand(
  runner: ModelMergeCommandRunner,
  command: Sbv2ModelMergeCommand,
  logLines: string[],
): Promise<void> {
...
function appendCommandOutput(logLines: string[], stream: "stdout" | "stderr", chunk: string): void {
  for (const line of chunk.split(/\r?\n/)) {
    ...
    if (trimmed) logLines.push(`${stream}: ${trimmed}`);
  }
}

function validateMethodParameters(options: ModelMergePlanOptions): void {
  if (methodNeedsModelC(options.method) && !options.modelC) throw new Error(`--model-c is required for ${options.method}`);
  if (!methodNeedsModelC(options.method) && options.modelC) throw new Error(`--model-c is only valid for add-diff and weighted-sum`);
  if (options.method !== "usual" && options.slerp) throw new Error("--slerp is only valid for usual");
  if (commandPayloadUsesWeights(options.method)) {
    normalizeWeights(options);
    if (hasCoefficients(options.coefficients)) throw new Error(`model coefficients are only valid for weighted-sum`);
  } else {
    normalizeCoefficients(options);
    if (hasWeights(options.weights)) throw new Error("part weights are not valid for weighted-sum");
  }
}
...
function hasWeights(value: Partial<Sbv2ModelMergeWeights> | undefined): boolean {
  return value !== undefined && Object.values(value).some((entry) => entry !== undefined);
}

function hasCoefficients(value: Partial<Sbv2WeightedSumCoefficients> | undefined): boolean {
  return value !== undefined && Object.values(value).some((entry) => entry !== undefined);
}

function methodNeedsModelC(method: Sbv2ModelMergeMethod): boolean {
  return method === "add-diff" || method === "weighted-sum";
}

function commandPayloadUsesWeights(method: Sbv2ModelMergeMethod): boolean {
  return method === "usual" || method === "add-diff" || method === "add-null";
}

function toUpstreamMethod(method: Sbv2ModelMergeMethod): string {
...
async function readConfigSummary(configJsonPath: string): Promise<ParsedConfigSummary> {
  await requireNonEmptyFile(configJsonPath, "config.json");
  const parsed = JSON.parse(await readFile(configJsonPath, "utf8")) as unknown;
  if (!isRecord(parsed) || !isRecord(parsed.data)) throw new Error(`config.json is missing data object: ${configJsonPath}`);
  const data = parsed.data;
  const spk2id = data.spk2id;
  if (!isRecord(spk2id) || Object.keys(spk2id).length === 0) throw new Error(`config.json data.spk2id must be a non-empty object: ${configJsonPath}`);
...
async function readNpyShape(filePath: string): Promise<number[]> {
  await requireNonEmptyFile(filePath, "style_vectors.npy");
  const buffer = await readFile(filePath);
  const header = parseNpy(buffer);
  if (!header || header.shape.length < 2) throw new Error(`style_vectors.npy is not a valid 2D NumPy file: ${filePath}`);
  if (!isSupportedFloatDescriptor(header.descr)) throw new Error(`style_vectors.npy dtype must be float32 or float64: ${filePath}`);
  const expectedBytes = header.shape.reduce((total, value) => total * value, 1) * header.bytesPerElement;
  if (buffer.length < header.dataOffset + expectedBytes) throw new Error(`style_vectors.npy data is truncated: ${filePath}`);
  return header.shape;
}
...
function validateStyle2idPermutation(style2id: Record<string, number>, size: number, label: string): void {
...
async function readSafetensorsTensorMap(filePath: string): Promise<Record<string, { dtype?: string; shape: number[] }>> {
  ...
  if (!header || !isRecord(header)) throw new Error(`safetensors file is not valid: ${filePath}`);
  const result: Record<string, { dtype?: string; shape: number[] }> = {};
  ...
  if (!Object.keys(result).length) throw new Error(`safetensors file has no tensor entries: ${filePath}`);
  return result;
}

async function readSafetensorsHeader(filePath: string, fileSize: number): Promise<unknown | undefined> {
  ...
  try {
    ...
    if (headerLength < 2 || headerLength > MAX_SAFETENSORS_HEADER_BYTES || 8 + headerLength > fileSize) return undefined;
    ...
    return JSON.parse(headerBuffer.toString("utf8")) as unknown;
  } finally {
    await file.close();
  }
}

async function requireNonEmptyFile(filePath: string, name: string): Promise<{ size: number }> {
  try {
    const fileStat = await stat(filePath);
    if (!fileStat.isFile() || fileStat.size === 0) throw new Error(`${name} is missing or empty: ${filePath}`);
    return { size: fileStat.size };
  } catch (error) {
    if (isNodeError(error) && (error.code === "ENOENT" || error.code === "ENOTDIR")) throw new Error(`${name} is missing or empty: ${filePath}`);
    throw error;
  }
}
...
function collectMergeArtifacts(plan: Sbv2ModelMergePlan, candidate: Sbv2ModelCandidate): string[] {
  return [
...
async function cleanupOutputDir(outputDir: string, logLines: string[]): Promise<void> {
  try {
...
function createStringNumberRecord(): Record<string, number> {
