...
function assertSbv2TextWithinHardLimit(text: string, maxInputChars: number): void {
  if (text.length <= maxInputChars) return;

  throw new Error(
    `SBV2 /voice text is too long: ${text.length} chars exceeds provider hard limit ${maxInputChars}. ` +
